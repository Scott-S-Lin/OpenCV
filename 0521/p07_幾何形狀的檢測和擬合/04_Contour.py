...
if __name__ =="__main__":
    ...
    img = cv2.imread('img7/frame36.jpg',cv2.IMREAD_GRAYSCALE)
    ...
    (im, contours, h)= cv2.findContours(binaryImg,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    #輪廓的數量
    n =  len(contours)
    contoursImg = []
    #畫出找到的輪廓
    ...
    #cv2.imshow("contour-"+str(i),contoursImg[32])#試看看改contoursImg的陣列數字
    cv2.waitKey(0)
